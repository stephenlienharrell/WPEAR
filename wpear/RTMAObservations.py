...
class RTMAObservations(WeatherData.WeatherData):
    ...
    def __init__(self):
        super(RTMAObservations, self)

        self.data_type = 'obs'
        self.url = 'http://www.ftp.ncep.noaa.gov'
        self.url_directory = '/data/nccf/com/rtma/prod/rtma2p5.%Y%m%d/'
        ...
        self.local_directory = 'web/%Y/%m/%d/rtma_obs/'
        # use strftime on datetime here
        self.local_secondary_directory = 'web/rtma_obs/%Y/%m/%d/'

        output_filename_format = 'rtma_obs_{time}_{vars}_{domain}_2dvaranl_ndfd.grb2'
